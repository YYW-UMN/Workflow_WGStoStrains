...
library(plyr)
library(reshape2)
AnalyzeRepeats <- function(farm,max.duplicate,meta.cols) {
	resultlist <- list() 
	for (c in (1:length(unique(farm$CowID)))) {
		cowID <- unique(farm$CowID)[c]
		cow <- farm[which(farm$CowID == cowID),]
		snp <- cow[,which(colSums(cow[,13:ncol(cow)]) > 0 & colSums(cow[,13:ncol(cow)]) < 998) + meta.cols]
                res <- matrix(0,nrow = nrow(cow),ncol = 16)
		baseline <- which(snp[1,] > 0)
                baseline.het <- which(snp[1,] >= 10) 
		for (s in (1:nrow(cow))){
			res[s,1] <- cow[s,'State']; res[s,2] <- cow[s,'CowID']
			res[s,3] <- rowSums(snp[s,] > 0) # tot
			res[s,4] <- rowSums(snp[s,] >0 & snp[s,] <= 4) # hom	
